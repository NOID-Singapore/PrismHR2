...
const AttendanceHistoryContent: FC = () => {
  ...
  const [isSearchingAttendanceHistory, setSearchingAttendanceHistory] = useState<boolean>(false);
  const [isSearchingAttendanceHistoryError, setSearchAttendanceHistoryError] = useState<boolean>(false);
  const [attendanceHistories, setAttendanceHistories] = useState<AttendancesModel[]>([]);
  ...
  const fetchData = useCallback(() => {
    ...
    const searchAttendanceHistory = async () => {
      setSearchingAttendanceHistory(true);
      setSearchAttendanceHistoryError(false);

      try {
        const url = `${ATTENDANCE_BASE_URL}?${getQueryParams()}`;
        const { data } = await axios.get(url, { cancelToken: cancelTokenSource.token });
        setCount(data.count);
        setAttendanceHistories(data.attendanceHistories);
      } catch (err) {
        setSearchAttendanceHistoryError(true);
      }

      setSearchingAttendanceHistory(false);
    };
  ...
  }, [rowsPerPage, currentPage, employeeId, queryString, filterBy, startDate, endDate]);
  ...
  const renderDetailHeaderLabel = () => {
    if (filterBy) {
      if (filterBy === 'shiftDate' && startDate && endDate) {
        return (
          <Typography variant='h6' color='primary'>
            (by Shift date {format(new Date(startDate), 'dd/MM/yyyy')} - {format(new Date(endDate), 'dd/MM/yyyy')})
          </Typography>
        );
      }
    }
  ...
  return (
    <Fragment>
        ...
        <Grid item xs={6}>
          <Grid container direction='row' justify='flex-end' alignItems='center' className={classes.rightHeader}>
            ...
            <PositionedPopper
              ...
              options={[{ key: 'shiftDate', label: 'Shift Date' }]}
              filterBy={filterBy}
      ...
      <AttendanceHistoryTable
        isLoadingData={isSearchingAttendanceHistory}
